...
const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
 ...
 //M code below:
  ...
  // try{
  //   const generate = jwt.sign(payload, secretPhrase,{ expiresIn: 3600 }, function(err, token){
...
const generateAuthTokens = async (user) => {
  const accessExpire = Math.floor(Date.now() / 1000) + config.jwt.accessExpirationMinutes * 60;

  const accessToken = generateToken(user.id, accessExpire, tokenTypes.ACCESS);
  
  return {
    access: {
      token: accessToken,
      expires: new Date(accessExpire * 1000)
    } 
  } 
};
